...
function createTask({ title, description, sprint_id, status, type, assignee_id, assigneeName, assigneeColor, priority, order, labelIds, due_date, estimate, dependsOnIds, archived, author }) {
  ...
  const finalOrder = order !== undefined && order !== null
    ...
    : (db.prepare(`
        ...
        WHERE status = ?
          AND archived = ?
      `).get(finalStatus, archived ? 1 : 0).next_order);
  ...
  const tx = db.transaction(() => {
    const finalOrder = order !== undefined && order !== null
      ...
      : (db.prepare(`
          ...
          WHERE status = ?
            AND archived = ?
        `).get(status, existing.archived ? 1 : 0).next_order);
    ...
    normalizeLaneOrder({ status: existing.status, archived: existing.archived });
    normalizeLaneOrder({ status, archived: existing.archived });
  });
  tx();
  ...
  const tx = db.transaction(() => {
    const destinationTasks = db.prepare(`
      ...
      WHERE status = ?
        ...
        AND id != ?
      ORDER BY "order" ASC, created_at DESC, id ASC
    `).all(status, existing.archived ? 1 : 0, id).map(t => t.id);
