...
function moveTask(id, status, order, author) {
  ...
  logActivity({ task_id: id, action: 'moved', old_value: existing ? existing.status : null, new_value: status, author: author });
  return getTask(id);
}
